...
def plot_stacked_bar_pandas_df(stackedbarplot_filepath, df, title, xlabel, ylabel, figsize=(18,8), label_size=13, title_size=20,
...
# args
	clean_counts_path = Path("/home/crisfbazz/Documents/CNPEM/NP3_MS_Data/Data_Collections/Bra346/bra_300626/outs/bra_300626/count_tables/clean/bra_300626_peak_area_clean_ann.csv")
	metadata_path = Path("/home/crisfbazz/Documents/CNPEM/NP3_MS_Data/Data_Collections/Bra346/bra_300626/Bra346_METADATA.csv")
	output_path = Path("/home/crisfbazz/Documents/CNPEM/NP3_MS_Data/Data_Collections/Bra346/bra_300626/outs/bra_300626/final_reports")

# post analysis routine for drug discovery research
# plot the mzs occurrence and superclass grouping by sample and save them to the output_path together with
...
# # topk is the number of samples to select and filter in the plots with top values of exclusive and not annotated m/zs - top novelty
# # rm_blanks, rm_beds and rm_controls receive a True or False to allow removing blanks, culture media or control samples and m/zs from the metrics computation
# # use_protonated a True of False defining if only the protonated m/zs should be used in the plots (filter the table with protonated_representative==1).
# # plots args for sizing personalization and legend size and position:
# # mzs_barplot (exclusive and redudant m/zs distribution with library annotation by sample),
# # donutplots of library annotation proportion and exclusive/redundant m/zs proportion in the selected samples
...
def post_dd_analysis_plots(metadata_path, clean_counts_path, output_path, topk = None, rm_blanks = True,
                           ...
                           mzs_barplot_legend_ncol=4, mzs_barplot_colors=None,
                           superclass_barplot_figsize=(20, 10), superclass_barplot_label_size=15,
                           superclass_barplot_title_size=20, superclass_barplot_legend_bbox=(0.5, -0.15),
	...
	if topk is None or topk <= 0 or topk > metadata_df.shape[0]:
		...
		print("- Top k novelty samples set to the number of samples in the provided metadata table = ", topk,
		      "\n  - Select the k samples with more exclusive and not annotated m/zs for plotting")
	
	if len(mzs_barplot_colors) < topk:
		print("WARNING: the provided list of colors in the mzs_barplot_colors parameter has a length smaller than the topk number of selected samples for plotting and will not be used. The default coloring will be used instead. ")
		mzs_barplot_colors = None
	...
	print("- Total number of m/zs in the provided clean table =", n)
	print("- Number of valid m/zs after filtering =", number_valid_mzs)
	if len(sample_type_rm) > 0:
		print("  - The m/zs appearing in the following sample types were removed by the filters: ",
		      ','.join(sample_type_rm))
	
	mzs_selected = "" # all mzs
	if use_protonated:
		if "protonated_representative" in clean_counts_df.columns:
			print("- Filtering only the protonated representative m/zs")
			clean_counts_df = clean_counts_df.loc[clean_counts_df.protonated_representative == 1, :]
			number_valid_mzs = clean_counts_df.shape[0]
			print("  - Number of valid m/zs after protonated filtering =", number_valid_mzs)
			mzs_selected = "[M+H]+ "  # protonated
			output_name = output_name+"_protonated"
		else:
			print("- The protonated_representative column is not present in the provided clean table, not filtering the protonated m/zs.")
	...
	# create a column to store the mzs with an identification annotation
	...
	if annotation_lib.sum() != number_valid_mzs:
		sys.exit("The number of total mzs (library annotated and not annotated) does not match the number of valid mzs. Something went wrong in the processing.")
	print("- Creating the distribution of library annotation:", annotation_lib.to_dict())
	
	plot_donut_values(Path(output_path,output_name+"_mzs_lib_annotation_dist.png"),
	                  values = annotation_lib[sorted(annotation_lib.index.values)].values,
	                  categories=sorted(annotation_lib.index.values),
	                  title = "Distribution of Library Annotations for the "+mzs_selected+"m/zs", colors=donutplot_libAnnotations_colors,
	...
	# count the number of mzs that appear in each sample (total), the number that are exclusive and/or redudant by sample
	# where the clean_samples_df is the clean count table with only the samples quantification columns
	# check the mzs that are present in each sample, which have a peak area > 0
	...
	# check the mzs that appear in only one sample, which are exclusive to a sample
	check_exclusive_mzs = (count_samples_by_mz == 1)
	# check the mzs that appear in more than one sample, which are redundant across the dataset
	check_redundant_mzs = (count_samples_by_mz > 1)
	# count the number of exclusive mzs by sample, which is present in the sample and is an exclusive mz
	exclusive_mzs_by_sample = present_mzs_by_sample.apply(lambda col: col & check_exclusive_mzs)
	
	# create a count table for the mzs with the total of mzs by sample, the number of exclusive mzs by sample and
	# # the number of redundant mzs by sample (which appear in more than one sample) for annotated and not annotated mzs
	...
	print("- Computing the mzs distribution occurrence by sample and creating some plots:\n  "+
	      "- number of exclusive m/zs (only appear in one sample);\n  "+
	      "- number of redundant m/zs (appear in more than one sample)\n  "+
	      "- number of annotated m/zs (received a curated library annotation from GNPS or UNPD).")
	# join the not annotated and the annotated mzs counts by sample
	# df_final_1
	...
	mzs_count_by_sample.to_csv(Path(output_path, output_name+"_mzs_dist_count_by_sample.csv"), index_label='SAMPLE_CODE')
	
	# norm by selected cols, sort by exclusive not annotated - possible novel compounds - and filter topk
	# where the top novelty samples are the samples with more exclusive not annotated m/zs
	...
	# plot exclusive and redundant m/zs by sample
	plot_stacked_bar_pandas_df(Path(output_path,output_name+"_mzs_distribution_top_"+str(topk)+"_samples_novelty.png"),
	                           mzs_count_by_sample_norm_topk,
	                           title='Exclusive and Redundant '+mzs_selected+'m/zs Distribution with Library Annotation for the Top '+str(topk)+' Novelty Samples',
	                           xlabel='Samples', ylabel='Percentage of detected '+mzs_selected+'m/zs',
	...
	plot_donut_values(Path(output_path,output_name+"_mzs_presence_dist_all_donutplot.png"),
	                  values = [check_exclusive_mzs.sum(), check_redundant_mzs.sum()],
	                  categories=["Exclusive", "Redundant"],
	                  title = "Distribution of Exclusive and Redundant "+mzs_selected+"m/zs", colors=donutplot_mzs_distr_colors,
	                  title_size=donutplots_title_size, text_size=donutplots_text_size)
	plot_donut_values(Path(output_path,output_name+"_mzs_presence_dist_annotated_donutplot.png"),
	                  values = [check_exclusive_mzs[clean_counts_df.annotated].sum(),
	                            check_redundant_mzs[clean_counts_df.annotated].sum()],
	                  categories=["Exclusive", "Redundant"],
	                  title = "Distribution of Exclusive and Redundant "+mzs_selected+"m/zs with Library Annotation",
	                  colors=donutplot_mzs_distr_colors,
	                  title_size=donutplots_title_size, text_size=donutplots_text_size)
	
	print("- Computing the superclass grouping distribution of the m/zs by sample and creating a plot.")
	...
	clean_samples_by_superclass_grouping.to_csv(Path(output_path, output_name+"_superclass_grouping_count_by_sample.csv"),
	                                            index_label='SAMPLE_CODE')
	
	class_colors = {'Alkaloids_and_Lactams': '#ff8b00',                    # Alaranjado
	                'Aminoacids_and_Peptides_and_OtherNComp': '#f1fd7d',   # Amarelo
	                'Benzenoids': '#6cf61e',                               # Verde claro
	                'Fatty_Acids_and_Lipids': '#24eb49',                   # Verde
	                'Flavonoids_and_Phenolic_derivatives': '#61dcba',      # Azul claro
	                'Lignans_and_Other_Ocompounds': '#0093c9',             # Azul
	                'Organic_Acids_and_OthersGenerals': '#6372b4',         # Azul escuro
	                'Polyketides': '#771981',                              # Violeta
	                'Terpenes_and_Carotenoids': '#cc0000',                 # Vermelho
	                'Not_Annotated': "#a8a8a8"}
	...
	clean_samples_by_superclass_grouping_topk = clean_samples_by_superclass_grouping.loc[selected_samples_topk,list(class_colors.keys())]
	...
	plot_stacked_bar_pandas_df(Path(output_path,output_name+"_superclassgrouping_dist_top_"+str(topk)+"_samples_novelty.png"),
	                           clean_samples_by_superclass_grouping_topk_norm,
	                           title='Superclass Grouping Distribution for the '+mzs_selected+'m/zs of the Top '+str(topk)+' Novelty Samples (normalized by sample)',
	                           xlabel='Samples', ylabel='Percentage of detected '+mzs_selected+'m/zs',
	                           colors=list(class_colors.values()),
	                           figsize=superclass_barplot_figsize, label_size=superclass_barplot_label_size,
	                           title_size=superclass_barplot_title_size,
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(
		description='Post analysis routine for drug discovery research using the NP3 results. Create plots with the m/zs occurrence and '+
	...
	parser.add_argument("--topk", default=None, type=int,
	                    help="The number of samples to be selected and filtered in the plots with the top values of exclusive and not annotated m/zs - top novelty samples. If None, disable filtering. This filtering does not affect the metrics computations, its only for visualization purposes.")
	parser.add_argument("--rm_blanks", default=True, type=str2bool,
	                    help="True or False to allow removing blank samples and m/zs from the metrics computation.")
	parser.add_argument("--rm_beds", default=True, type=str2bool,
	                    help="True or False to allow removing culture media samples and m/zs from the metrics computation.")
	parser.add_argument("--rm_controls", default=False, type=str2bool,
	                    help="True or False to allow removing control samples and m/zs from the metrics computation.")
	parser.add_argument("--use_protonated", default=False, type=str2bool,
	                    help="True of False defining if only the putative [M+H] m/zs should be used in the output tables and plots (filter the table with protonated_representative == 1). This will affect the metrics computation.")
	# plots parms
	...
	parser.add_argument("--donutplot_libAnnotations_colors", default='#ff8b00,#6372b4,#c6c6c6', type=str2tlist,
	                    help="The list of colors separated by comma for the library annotation distribution donut plot. Three colors are expected for 'GNPS', 'UNPD' and 'not_annotated' categories.")
	parser.add_argument("--donutplot_mzs_distr_colors", default='#0072c3,#42be65', type=str2tlist,
	                    help="The list of colors separated by comma for the mzs occurrence distribution donut plot. Two colors are expected for 'Exclusive' and 'Redundant' categories.")
	parser.add_argument("--mzs_barplot_figsize", default='18,8', type=str2ilist,
	                    help="The x,y figure size of the mzs occurrence distribution in a stacked bar plot by sample. Two integer values separated by comma.")
	parser.add_argument("--mzs_barplot_label_size", default=13, type=int,
	                    help="The axis label size of the mzs occurrence distribution in a stacked bar plot by sample.")
	parser.add_argument("--mzs_barplot_title_size", default=20, type=int,
	                    help="The title size of the mzs occurrence distribution in a stacked bar plot by sample.")
	parser.add_argument("--mzs_barplot_legend_bbox", default='0.5,-0.2', type=str2flist,
	                    help="The x,y anchoring coordinates relative to the plot area of the mzs occurrence distribution in a stacked bar plot by sample. Two float values separated by comma. (0, 0) is the bottom-left corner of the plot. (1, 1) is the top-right corner of the plot. Values smaller than 0 or greater than 1 will place the legend completely outside the plot area.")
	parser.add_argument("--mzs_barplot_legend_fontsize", default=17, type=int,
	                    help="The legend font size of the mzs occurrence distribution in a stacked bar plot by sample.")
	parser.add_argument("--mzs_barplot_legend_ncol", default=4, type=int,
	                    help="The number of columns to display the legends of the mzs occurrence distribution in a stacked bar plot by sample.")
	parser.add_argument("--mzs_barplot_colors", default=None, type=str2tlist,
	                    help="The list of colors of the mzs occurrence distribution in a stacked bar plot by sample. One color by samples is expected. If None, use the default matplot coloring.")
	...
	parser.add_argument("--superclass_barplot_legend_fontsize", default=15, type=int,
	                    help="The legend font size of the superclass distribution in a stacked bar plot by sample.")
