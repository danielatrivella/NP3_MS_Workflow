...
if (length(args) < 1) {
  stop("One argument must be supplied to clean and annotate the counts from joined jobs:\n", 
       " 1 - Path to the output data folder, inside the outs directory of the clustering result folder. ", 
       "It should contain the mgf folder, the count_tables dir with the peak area count and the spectra count in CSVs. The data name will be extracted from here.;\n", 
       " 2 - The precursor m/z tolerance in Da;\n",
       " 3 - The similarity tolerance to JOIN mass dissipation spectra;\n",
       " 4 - The retention time tolerance to enlarge the peaks before comparisions;\n",
       " 5 - The bin size in Da to consider two fragmented peaks m/z's the same;\n",
       " 6 - The scale factor to be used to aggregated the peaks of joined clusters;\n",
       " 7 - The ionization mode, one of -1 or 1 (default to 1);\n",
       " 8 - The bflag_cutoff a positive numeric or FALSE to disable it. If positive numeric, allow to join the spectra with a low basePeakInt (<= median + bflag_cutoff*IQR) with a blank spectra without relying on the similarity value;\n",
       " 9 - The NOISE_cutoff a positive numeric or FALSE to disable it. If positive numeric, allow to remove the spectra with a low basePeakInt  (<= median + NOISE_cutoff*IQR);\n",
       " 10 - The maximum number of spectra (rows) to be processed at a time.",
       call.=FALSE)
} else {
  ...
  path_metadata_orig_samples <- file.path(output_path, "../..", 
                                          "original_samples_METADATA.csv")
  if (!file.exists(path_metadata_orig_samples))
  {
    stop("The metadata with the original samples from the joined jobs '", path_metadata_orig_samples,
  ...
  if (length(args) > 9) {
    ...
    sim_tol <- as.numeric(args[[3]]) # sim to join
    if (is.na(sim_tol))
    ...
    BFLAG_cutoff_factor <- args[[8]]  # check if it is FALSE
    if (BFLAG_cutoff_factor != "FALSE") { 
      # check if it is numeric
      BFLAG_cutoff_factor <- as.numeric(args[[8]]) 
      ...
      BFLAG_cutoff_factor <- as.logical(args[[8]]) 
    }
    
    NOISE_cutoff_factor <- args[[9]]  # check if it is FALSE
    if (NOISE_cutoff_factor != "FALSE") { 
      # check if it is numeric
      NOISE_cutoff_factor <- as.numeric(args[[9]]) 
      ...
      NOISE_cutoff_factor <- as.logical(args[[9]]) 
    }
...
# create the clean similarity file - aggMax
...
# read the metadata with the original samples that are being joined
metadata_orig_samples <- readMetadataTable(path_metadata_orig_samples)

# read count files, remove tremolo columns
...
# create the joinedIDs columns to store the IDs joined in the current clean step 
# and the numJoins to count the number of joined spectra by m/z in the clean step
...
not_count_columns <- which(!(names(ms_spectra_count) %in% paste0(metadata_orig_samples$SAMPLE_CODE, "_spectra"))) 
count_columns <- which(names(ms_spectra_count) %in% paste0(metadata_orig_samples$SAMPLE_CODE, "_spectra"))
...
warnings()
cat("\n\n  ** WARNINGS END** \n\n")
cat("\n  ** Computing Peak Area by Sample for the Joined Jobs ** \n\n")
...
blanks_code <- metadata_orig_samples[metadata_orig_samples$SAMPLE_TYPE == "blank", "SAMPLE_CODE"]
...
controls_code <- metadata_orig_samples[metadata_orig_samples$SAMPLE_TYPE == "control", "SAMPLE_CODE"]
...
bed_controls_code <- metadata_orig_samples[metadata_orig_samples$SAMPLE_TYPE == "bed", "SAMPLE_CODE"]
