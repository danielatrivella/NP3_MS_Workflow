...
# where 
# @putative_origin indicates if the column with the msclusterID of the m/z 
...
# @with_num_joins indicates to search for fragmented clusters only in the rows where num_joins > 0 (used in the clean step)
compute_fragmented_clusters <- function(clean_count_table, 
                                        rt_tol=2, mz_tol=0.025, 
                                        putative_origin = FALSE, 
                                        table_type="Step 5 clean",
                                        with_numJoins=FALSE,
                                        add_space_verbose="") {
  cat("\n",add_space_verbose,"* Computing the number of fragmented cluster in the",table_type,"table *\n")
  t1 <- Sys.time()  
  ...
  for (i in seq_len(nrow(clean_count_table))) {
    ...
    if (with_numJoins && clean_count_table$numJoins[[i]] == 0) {
      next()
    }
  ...
  cat("\n",add_space_verbose,"  - Total number of m/zs that had at least one putative fragmented clusters and percentage overall m/zs:", 
      number_mzs_w_fragmented_clusters, "(",round(100*number_mzs_w_fragmented_clusters/nrow(clean_count_table),2), "% ) \n")
  cat("",add_space_verbose,"  - Total number of putative fragmented clusters and percentage overall m/zs:", 
      number_fragmented_clusters, "(",round(100*number_fragmented_clusters/nrow(clean_count_table),2), "% ) \n")
  
  t2 <- Sys.time()
  cat("",add_space_verbose,"  * Done in", round(t2-t1, 2), units(t2-t1), "*\n")
