...
# then the library search result can be enriched offline with all these retrieved info - workaround to execute np3 offline and remove dependency to the GNPS server for annotation enrichment of the identifications
# this should be updated by the adm everytime the Library updates (new spectrumIDs are added) - a warning will show up in the joining step from NP3 setup when a new updated is needed
