...
## 4.11. Command *join_jobs* 
  ...
  Command to join original $NP^{3}$ jobs (results of the run command) into a single united job. Concatenate different jobs without the need of running them all together again. Uses a different metadata, called *metadata_join*, defining the jobs to be joined and their reference codes, the names of their original metadata and pre processing directory. Use the clean results from the original jobs and execute the pipeline from step 3 to the end with some modifications and adaptations. 
...
joining $NP^{3}$ results, coming from the *run* or the *join_job* commands.
...
- A folder named 'outs' with the clustering result of the joined jobs in a single subfolder named with the *output_name* containing:
  - A subfolder named 'count_tables' with the Step 4 quantifications in CSV tables named as '<step_name>_(spectra|peak_area).csv'. And inside it the clean tables in a folder named 'clean'.
...
##### 4.11.1. *join_jobs* Metadata_join creation
  ...
  The *metadata_join* table defines the $NP^{3}$ jobs to be united, it defines the joining jobs reference names and original pre processing result. It also defines if the joining jobs came from a *run* or a *join_jobs* commands result. 
...
- JOBNAME \- TODO the raw data file names (including extensions but not including complete paths). The accepted file format is mzXML or mzML (mzData format could also be used, but was not vastly tested).
- JOB_CODE \- TODO unique and syntactically valid (see below) names for each file. The use of the same SAMPLE_CODE in more than one entry will generate an error. A syntactically valid name must start with a letter and consist of letters, numbers, and underscore characters. R reserved words (https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Reserved-words) are not syntactically valid names. The SAMPLE_CODE names will be used to name and reference the pre-processed files in Step 2 and to name the output count table's columns of Steps 3 to 9.
- METADATA_NAME \- TODO
- PRE_PROCESSED_DATA_NAME \- TODO
- JOINED_JOB \- TODO
 

The samples' groupings, correlations and bioactivities will be retrieved from the original jobs samples metadata. There information is not defined here.
...
TODO 3 coisas
Um novo metadata contendo os jobs a serem unidos​

Uma pasta contendo o resultado do np³ dos jobs a serem unidos:​
...
Uma pasta contendo o pre process dos jobs originais sendo unidos​

Pre_process_join_jobs/​
