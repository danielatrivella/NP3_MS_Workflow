...
## 4.11. Command *join_jobs* 
  ...
  Command to join original $NP^{3}$ jobs (results of the run command) into a single united job. Concatenate different jobs without the need of running them all together again. Uses a different metadata, called *metadata_join*, defining the jobs to be joined and their reference codes, the names of their original metadata and pre processing directory. It uses the clean results from the original jobs and execute the pipeline from step 3 to 10 with some modifications and adaptations, except for step 8 which is skipped. 
  
  This command performs the clustering of all the clean data from the original jobs together, then proceed the workflow to the clean step, next update the original ionization annotations with the final joined clean consensus spectra and merge the original IVAMNS, compare the final joined consensus spectra pairwise, skip merge step, finally compute the molecular networking with the joined result and compute the biocorrelation using the original samples metadata of all original jobs together.
...
joining $NP^{3}$ results, coming from the *run* or the *join_job* commands. The user must only ensure that all sample's code is unique across the different original jobs being joined (SAMPLE_CODE column from the original samples' metadata).

The following subsections define how to create the metadata_join, its expected format, the data organization to execute this command and details on its methods.
...
2. A folder containing the results of the original $NP^{3}$ jobs to be united or united by any included joined job, all jobs defined in the metadata_join must be placed inside this folder (parameter *jobs_data_path*). The data to be united will be retrieved from here.
3. And a folder containing the results of the pre processing of the original $NP^{3}$ jobs to be united or united by any included joined job (paramater *pre_processed_dir_path*). The data to correctly compute the peak area of the final joined clean consensus spectra will be retrieved from here.
...
TODO
steps difference
peak area recalculations for correct values

how the annotate_protonated step 7 occurs

why merge is skipped and that it can be executed a posteriori

- the sample codes from the original jobs being united must be unique among all joined jobs, this will through an error if not true.
