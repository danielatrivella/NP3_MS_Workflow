...
The $NP^{3}$ MS workflow is included in the GNPS2 Workflows and may be executed online using their web interface. This interface includes only the pre_process+run command functionality and the most important parameters. Search for "np3" in their workflows list: https://gnps2.org/workflows.
...
The NPClassifier [21], NPAtlas [22] and ClassyFire [23] solutions were used to add origin and class information to the UNPD compounds. $NP^{3}$ MS Workflow will retrieve this information together with the identification results.
 
At the end of the identification step, a curation is performed to select the best result from the 
top_k identifications of each spectra and to score them with a personalized
criteria. Additionally, a superclass grouping of the NPClassifier result was proposed by the $NP^{3}$ team and is also applied here for the final curated superclass.
...
| curated_lib_annotation_superclass_GR_\<curated superclass group\> | 0 or +1 values indicating if the respective curated superclass group appears in the curated_lib_annotation_superclass_grouping zero, one or more times as detailed in Section 4.6.2|
...
##### 4.13.4. *gnps_result* Identification Curation and UNPDxGNPS Final Curation

A curation of the identification results was also proposed for the gnps_result in order to score and rank the best identifications from the GNPS libraries. The curation is executed after the gnps result is merged to the $NP^{3}$ count tables.
...
Then, after the gold results were selected, the curation proceeds to a categorization and scoring of the GNPS result, similarly to the Identification Curation of UNPD-tremolo. Ten categories and scores were defined to group the best results according to the reliability of their matches or to put them out of the analysis, using the criteria presented below:
