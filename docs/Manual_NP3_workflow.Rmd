...
The Identification Curation results are concatenated to these created column, detailed below.
...
```{r, eval=F} 
  $ node np3_workflow.js gnps_result --cluster_info_path 
    ...
    --ms_count_path "output_path/outs/output_name/count_files/clean/count_table.csv" 
    --output_path "/path/to/the/output/NP3/output_name/"
```
...
At the end, the joined GNPS results are also stored to a separated file called 'gnps_results_smiles.csv' inside the output_path/identifications folder. It will be used to compute the CDK descriptors for the unique SMILES from the GNPS result.
...
At the end of the GNPS curation step, the final $NP^{3}$ result will have the best curated identification from UNPD and GNPS. If the output_path is informed, the command further creates a chemical report with the GNPS and UNPDxGNPS identification statistics and stores to the final_reports/chemical_reports folder inside the output_path. In the chemical report, it also plots the chemical space with the curated GNPS identification and the best origin identification (UNPDxGNPS). 

For the chemical space plotting using PCA, the CDK descriptors of the GNPS result were previous computed and store in the output_path/identifications folder in file 'gnps_results_smiles_descriptorsCDK.csv'. The file 'gnps_results_smiles.csv' is previous created in this folder and used for the CDK descriptors computation. The top 24 descriptors are computed here for each unique SMILES present in the gnps_Smiles column.
